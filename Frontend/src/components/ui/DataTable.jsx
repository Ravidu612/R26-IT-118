...
function DataTable({ columns, rows, isLoading = false, error = '', emptyState, renderRowActions }) {
  if (isLoading) {
    return (
      <div className="rounded-[12px] border border-[var(--border-color)] bg-slate-900/55 p-6">
  ...
  if (error) {
    return (
      <div className="rounded-[12px] border border-rose-300/30 bg-rose-300/10 p-4 text-sm text-rose-100">
        {error}
      </div>
    )
  }
  ...
  return (
    <div className="overflow-x-auto rounded-[12px] border border-[var(--border-color)] bg-slate-900/45">
      <table className="min-w-full text-left text-sm text-slate-200">
        ...
        <tbody>
          {rows.map((row, index) => (
            <tr key={row.id || `${row.module || 'row'}-${index}`} className="border-t border-[var(--border-color)] align-top transition-colors hover:bg-emerald-50/40">
              {columns.map((column) => (
